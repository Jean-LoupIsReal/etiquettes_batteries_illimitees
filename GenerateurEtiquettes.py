...
class GenerateurEtiquettes:
    ...
    def dessiner_grille(self, c, hauteur_etiquette, largeur_etiquette):
        ...
        for i in range(self.nb_par_colonne + 1):
            ...
            c.line(self.marge_externe, y, largeur_etiquette * self.nb_par_ligne, y)
    ...
    def print_prix_separe(self, c, prix, x_etiquette, y_etiquette):
        ...
        l_section_desc = self.etiquette.dimensions["largeur"]
        ...
        paragraph_cent.wrapOn(c, largeur_dollar,  max_h_dollars)
        paragraph_dollars.wrapOn(c, largeur_cent, max_h_dollars)
        # Calcul emplacement prix.
        x_pos_dollars = x_etiquette + l_section_desc + self.etiquette.dimensions["marge_interne_x"]
    ...
    def generer_moyenne_grande_etiquettes(self, c, data, variante = False):
        ...
        for row in data.iterrows():
            ...
            w = self.etiquette.dimensions["l_section_prix"] - self.etiquette.dimensions["marge_interne_x"] * 2
            # S'assure que le code du produit reste dans le frame
            framed_code = KeepInFrame(w, h, [p_code], mode='shrink')
            framed_code.wrapOn(c, w, h)
            framed_code.drawOn(c, x + self.etiquette.dimensions["marge_interne_x"], y + self.etiquette.dimensions["marge_interne_y"])
            ...
            # Description
            p_desc = Paragraph(str(row[1].Description), self.etiquette.styles["style_description"])
            p_desc.wrapOn(c, l_section_desc, (y_desc - 2 * self.etiquette.dimensions["marge_interne_y"]) * 3/4 )
            ...
            if self.etiquette.prix_est_separe:
                self.print_prix_separe(c, row[1].Coûtant, x, y)
            else :
