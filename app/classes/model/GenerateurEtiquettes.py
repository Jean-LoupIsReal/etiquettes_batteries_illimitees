...
class GenerateurEtiquettes:
    ...
    def generer_pdf_etiquettes(self, data, nom_fichier_pdf="etiquettes.pdf"):
        
        pdf_path = f"{nom_fichier_pdf}"# Marges et dimensions
        c = canvas.Canvas(pdf_path, pagesize=(self.largeur_page, self.hauteur_page))

        # Demander le format de l'étiquette
        type_etiquette = None
        r = False
        print("===========================================\n" \
            "Sous quel format voulez vous imprimer l'etiquette?\n" \
            "1 : Petite \n" \
            "2 : Petite inversé\n" \
            "3 : Moyenne\n" \
            "4 : Grande\n" \
            "cancel ou c = menu principal\n")
        user_input = input("Reponse : ")
        user_input = user_input.lower()
        match user_input :
            case "1":
                print("petite_etiquette")
                type_etiquette = "petite_etiquette"
            case "2":
                print("petite inverse")
                type_etiquette = "petite_etiquette"
                r = True
            case "3":
                print("moyenne")
        ...
        c.setDash(1,0)
        c.setStrokeColor(colors.gray)   
        if type_etiquette == "petite_etiquette":
            self.generer_P_etiquettes(c, data, r)
        else : 
            self.generer_MG_etiquettes(c, data, r)

        # Création du pdf
        c.save()
        print(f"✅ PDF enregistré à : {nom_fichier_pdf}")

    def generer_P_etiquettes(self, c, data, variante = False):
        i = 0
    ...
    def generer_MG_etiquettes(self, c, data, variante = True):
        ...
        for row in data.iterrows():
            ...
            barcode = code128.Code128(str(row[0]), barHeight= h_code_bar, humanReadable=False)
            if pd.notna(row[0]):
                x_barcode = x + l_section_desc + (self.etiquette.dimensions["l_section_prix"] - barcode.width) / 2 if self.etiquette.dimensions["l_section_prix"] != 0 else x + (l_section_desc - barcode.width) / 2
                barcode.drawOn(c, x_barcode, y + 1)
            # Prix (très gros)
    ...
    def paragraphe_ajuste_produit(self, text, width, height_max = 0):
